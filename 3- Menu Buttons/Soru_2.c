...
// "Onay" butonuna bastýkça ledlerde 0-255 arasý binary sayma iþlemini gerçekleþtiriniz.//
...
void main()                                            // Ana program bloðu.
{
  ...
  while(1)                                             // Sonsuz Döngü
  {
    ...
    {
      ledler++;                                        // ledler degiskenini bir arttýr.
      GPIOE_ODR=ledler;                                // E portuna ledler degiskenini yükleyerek istenilen ledlerin yanmasini sagla.
      delay_ms(100);                                   // Buton arkini önlemek için 100 ms bekle.
      while(orta);
      delay_ms(100);
  }
